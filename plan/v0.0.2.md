...
- [ ] Create `ChromaProcessManager` service class
- [ ] Implement server lifecycle management (start/stop)
- [ ] Add health check mechanism with retry logic
- [ ] Implement automatic port allocation
- [ ] Handle graceful shutdown on extension deactivate
...
- [ ] Add binaries to extension package under `bin/` directory
- [ ] Implement platform detection logic
- [ ] Handle executable permissions on Unix systems

#### 1.3 Persistent Storage
- [ ] Store ChromaDB data in VS Code globalStorage
- [ ] Implement data persistence across extension restarts
...
- [ ] `semanticSearch.chroma.port`: Port for Chroma server (0 = auto-assign)
- [ ] `semanticSearch.chroma.logLevel`: Chroma server log level
- [ ] `semanticSearch.chroma.startupTimeout`: Timeout for server startup
...
- [ ] Show Chroma server status (running/stopped/error)
